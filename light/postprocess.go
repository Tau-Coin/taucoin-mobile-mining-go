...
import (
	...
	"github.com/Tau-Coin/taucoin-mobile-mining-go/common/bitutil"
...
func GetChtRoot(db taudb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db taudb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    taudb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db taudb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
...
func GetBloomTrieRoot(db taudb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db taudb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db taudb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
