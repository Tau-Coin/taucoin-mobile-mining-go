...
import (
	...
	"github.com/Tau-Coin/taucoin-mobile-mining-go/les/flowcontrol"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 1:
			...
			// bankUser transfers more tauer to user1
		...
		case 2:
			// bankUser transfer some tauer to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db taudb.Database, odr light.OdrBackend, config *light.IndexerConfig) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms)
	indexers[1] = tau.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db taudb.Database, peers *peerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightTau{
		lesCommons: lesCommons{
			...
			config:      &tau.Config{LightPeers: 100, NetworkId: NetworkId},
			chainConfig: params.AllTauashProtocolChanges,
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db taudb.Database, peers *peerSet, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllTauashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
