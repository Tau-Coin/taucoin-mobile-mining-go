...
const (
	...
	tauVersion        = 63              // equivalent tau version for the downloader
...
type serverHandler struct {
	...
	synced  func() bool    // Callback function used to determine whtauer local node is synced.

	// Testing fields
	addTxsSync bool
}

func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb taudb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *peer) error {
	...
	p.Log().Debug("Light Tau peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
		p.Log().Debug("Light Tau handshake failed", "err", err)
		return err
	}
	...
	if !h.server.clientPool.connect(p, 0) {
		p.Log().Debug("Light Tau peer registration failed", "err", errFullClientPool)
		return errFullClientPool
	}
	// Register the peer locally
	if err := h.server.peers.Register(p); err != nil {
		h.server.clientPool.disconnect(p)
		p.Log().Error("Light Tau peer registration failed", "err", err)
		return err
	}
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light tau response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Tau message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *peer, wg *sync.WaitGroup) error {
	...
	// accept returns an indicator whtauer the request can be served.
