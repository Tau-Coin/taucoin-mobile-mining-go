...
import (
	...
	"github.com/Tau-Coin/taucoin-mobile-mining-go/p2p/simulations"
...
// similar requests at the maximum allowed rate and the test verifies whtauer the
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "tau_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	err := client.CallContext(c, &res, "tau_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	tau, err := tau.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(tau, &config)
	if err != nil {
		return nil, err
	}
	tau.AddLesServer(server)
	return tau, nil
}
