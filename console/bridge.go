...
// RPC method (saved in jtau.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jtau.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jtau.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	case strings.HasSuffix(err.Error(), scwallet.ErrPairingPasswordNeeded.Error()):
		...
		if val, err = call.Otto.Call("jtau.openWallet", nil, wallet, passwd); err != nil {
...
func (b *bridge) readPassphraseAndReopenWallet(call otto.FunctionCall) (otto.Value, error) {
	...
	return call.Otto.Call("jtau.openWallet", nil, wallet, passwd)
}
...
// original RPC method (saved in jtau.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jtau.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// jtau.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jtau.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("tau.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
