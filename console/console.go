...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Tau requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jtau", struct{}{})

	jtauObj, _ := c.jsre.Get("jtau")
	jtauObj.Object().Set("send", bridge.Send)
	jtauObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jtau);"); err != nil {
	...
	flatten := "var tau = web3.tau; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = tau.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jtau.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jtau.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jtau.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jtau.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jtau.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines tau.getBalance(tau.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gtau instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gtau JavaScript console!\n\n"

	// Print some generic Gtau metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + tau.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + tau.blockNumber + " (" + new Date(1000 * tau.getBlock(tau.blockNumber).timestamp) + ")\n";
		try {
