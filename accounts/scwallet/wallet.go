...
type Wallet struct {
	...
	deriveChain     tau.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here becase we don't know whtauer
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whtauer
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Tau account
...
// Contains returns whtauer an account is part of this particular wallet or not.
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain tau.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
func (w *Wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
    ...
    isSign,err := (*tx).WithSignature(signer, sig)
    if isSign {
        return tx,nil
    }
	return nil,err
...
type Session struct {
	...
	verified bool                  // Whtauer the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whtauer the card has been initialized with a private key
}
...
func (s *Session) derive(path accounts.DerivationPath) (accounts.Account, error) {
	...
	return s.Wallet.makeAccount(crypto.PubkeyToAddress(crypto.PublicKey(*pub)), path), nil
}
