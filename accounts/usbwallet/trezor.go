...
import (
	...
	"github.com/Tau-Coin/taucoin-mobile-mining-go/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whtauer the device is waiting for PIN entry
	passphrasewait bool          // Flags whtauer the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whtauer the Trezor is opened, closed
// or whtauer the Tau app was not started on it.
...
// Tau address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.TauAddress)
	if _, err := w.trezorExchange(&trezor.TauGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.TauTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.TauTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
