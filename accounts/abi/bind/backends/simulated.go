...
type SimulatedBackend struct {
	database   taudb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // Tau blockchain to handle the consensus
...
func (b *SimulatedBackend) rollback() {
	blocks, _ := core.GenerateChain(b.config, b.blockchain.CurrentBlock(), tauhash.NewFaker(), b.database, 1, func(int, *core.BlockGen) {})
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call tau.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call tau.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call tau.CallMsg) (uint64, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call tau.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, uint64, bool, error) {
...
func (b *SimulatedBackend) FilterLogs(ctx context.Context, query tau.FilterQuery) ([]types.Log, error) {
...
func (b *SimulatedBackend) SubscribeFilterLogs(ctx context.Context, query tau.FilterQuery, ch chan<- types.Log) (tau.Subscription, error) {
...
func (fb *filterBackend) ChainDb() taudb.Database  { return fb.db }
