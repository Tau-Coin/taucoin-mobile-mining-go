...
func decryptPreSaleKey(fileContent []byte, password string) (key *Key, err error) {
	...
	/*
		See https://github.com/tau/pytausaletool

		pytausaletool generates the encryption key from password by
	...
	*/
	...
	tauPriv := crypto.Keccak256(plainText)
	ecKey := crypto.ToECDSAUnsafe(tauPriv)

	key = &Key{
		Id:         nil,
		Address:    crypto.PubkeyToAddress(crypto.PublicKey(ecKey.PublicKey)),
		PrivateKey: ecKey,
	}
	derivedAddr := hex.EncodeToString(key.Address.Bytes()) // needed because .Hex() gives leading "0x"
	expectedAddr := preSaleKeyStruct.TauAddr
