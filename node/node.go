...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (taudb.Database, error) {
	if n.config.DataDir == "" {
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string) (taudb.Database, error) {
	if n.config.DataDir == "" {
		return rawdb.NewMemoryDatabase(), nil
	}
