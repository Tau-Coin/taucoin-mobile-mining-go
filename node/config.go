...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gtau is "gtau". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gtau", keep that.
	if name == "gtau" || name == "gtau-testnet" {
		name = "Gtau"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gtau" instances.
var isOldGtauResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gtau 1.4 are used if they exist.
	if warn, isOld := isOldGtauResource[path]; isOld {
		oldpath := ""
		if c.name() == "gtau" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGtauResourceWarning, "Using deprecated resource file %s, please move this file to the 'gtau' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-tau-keystore")
		ephemeral = keydir
	}
