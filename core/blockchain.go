...
// Package core implements the Tau consensus protocol.
package core
...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Whtauer to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Whtauer to disable trie write caching and GC altogtauer (archive node)
...
func NewBlockChain(db taudb.Database, ipfsDb taudb.IpfsStore, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	...
	bc := &BlockChain{
		...
		stateCache:     state.NewDatabaseWithCache(ipfsDb, cacheConfig.TrieCleanLimit),
	...
	// the head block (tauhash cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) loadLastState() error {
    ...
    log.Info("load last state", "block root", currentBlock.Root())
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db taudb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db taudb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) CompleteHeaderChainWithBlock(blockChain types.Blocks, ancientLimit uint64) (int, error) {
	...
	// and returns a indicator whtauer the inserted blocks are canonical.
	...
	writeAncient := func(blockChain types.Blocks) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Gtau
	...
	writeLive := func(blockChain types.Blocks) (int, error) {
		...
		for i, block := range blockChain {
			...
			stats.processed++
			if batch.ValueSize() >= taudb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - taudb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have somtauing meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, []interface{}, error) {
	...
	for ; block != nil && err == nil; block, err = it.next() {
		...
		case CanonStatTy:
			log.Debug("Inserted new block", "number", block.Number(), "hash", block.Hash(),
				"txs", len(block.Transactions()), "elapsed", common.PrettyDuration(time.Since(start)),
				"root", block.Root())
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator) (int, []interface{}, error) {
	var (
		externTd *big.Int
		current  = bc.CurrentBlock()
	)
...
//
// The verify parameter can be used to fine tune whtauer nonce verification
