...
type Matcher struct {
	...
	running uint32 // Atomic flag whtauer a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togtauer.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togtauer
				var orVector []byte
...
// the requested retrieval queue to be serviced togtauer with other sessions.
//
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somtauing must arrive before we're aborting
