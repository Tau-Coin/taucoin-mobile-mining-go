...
func NewDatabase(db taudb.KeyValueStore) taudb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db taudb.KeyValueStore, freezer string, namespace string) (taudb.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains somtauing, ensure that the genesis blocks
...
func NewMemoryDatabase() taudb.Database {
...
func NewMemoryDatabaseWithCap(size int) taudb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (taudb.Database, error) {
...
func InspectDatabase(db taudb.Database) error {
	it := db.NewIterator()
	defer it.Release()
