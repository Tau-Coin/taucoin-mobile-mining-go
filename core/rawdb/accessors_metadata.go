...
func ReadDatabaseVersion(db taudb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db taudb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db taudb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db taudb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadPreimage(db taudb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db taudb.KeyValueWriter, preimages map[common.Hash][]byte) {
