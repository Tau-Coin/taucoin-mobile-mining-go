...
//   reserving it for go-tau. This would also reduce the memory requirements
//   of Gtau, and thus also GC overhead.
type freezer struct {
...
// HasAncient returns an indicator whtauer the specified ancient data exists
// in the freezer.
...
func (f *freezer) freeze(db taudb.KeyValueStore) {
	...
	for {
		...
		// Log somtauing friendly for the user
		context := []interface{}{
