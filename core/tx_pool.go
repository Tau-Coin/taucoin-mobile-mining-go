...
type TxPoolConfig struct {
	...
	NoLocals  bool             // Whtauer local transaction handling should be disabled
...
type TxPool struct {
	...
	istanbul bool // Fork indicator whtauer we are in the istanbul stage.
...
// validateTx checks whtauer a transaction is valid according to the consensus
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	*/
	...
	if pool.currentState.GetNonce(from) > (*tx).GetNounce() {
...
// and returns whtauer it was inserted or an older was better.
//
// Note, this method assumes the pool lock is held!
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) bool {
	...
	pool.pendingNonces.set(addr, (*tx).GetNounce()+1)
...
func (pool *TxPool) Status(hashes []common.Hash) []TxStatus {
	...
	for i, hash := range hashes {
		if tx := pool.all.Get(hash); tx != nil {
			from, _ := types.Sender(pool.signer, tx) // already validated
			if pool.pending[from] != nil && pool.pending[from].txs.items[(*tx).GetNounce()] != nil {
...
func (pool *TxPool) removeTx(hash common.Hash, outofbound bool) {
	...
	if pending := pool.pending[addr]; pending != nil {
		if removed, invalids := pending.Remove(tx); removed {
			...
			pool.pendingNonces.setIfLower(addr, (*tx).GetNounce())
...
func (pool *TxPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*txSortedMap) {
	...
	for addr, list := range pool.pending {
		...
		pool.pendingNonces.set(addr, (*txs[len(txs)-1]).GetNounce()+1)
	}
	pool.mu.Unlock()
			...
			for pending > pool.config.GlobalSlots && pool.pending[offenders[len(offenders)-2]].Len() > threshold {
				for i := 0; i < len(offenders)-1; i++ {
					...
					for _, tx := range caps {
						...
						pool.pendingNonces.setIfLower(offenders[i], (*tx).GetNounce())
