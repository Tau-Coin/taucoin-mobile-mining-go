...
// PublicTauAPI provides an API to access Tau full node-related
// information.
type PublicTauAPI struct {
	e *Tau
}

// NewPublicTauAPI creates a new Tau protocol API for full nodes.
func NewPublicTauAPI(e *Tau) *PublicTauAPI {
	return &PublicTauAPI{e}
}

// Tauerbase is the address that mining rewards will be send to
func (api *PublicTauAPI) Tauerbase() (common.Address, error) {
	return api.e.Tauerbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Tauerbase)
func (api *PublicTauAPI) Coinbase() (common.Address, error) {
...
func NewPublicMinerAPI(e *Tau) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Tau) *PrivateMinerAPI {
...
// SetFeeFloor sets the minimum accepted gas price for the miner.
func (api *PrivateMinerAPI) SetFeeFloor(gasPrice hexutil.Big) bool {
	api.e.lock.Lock()
	api.e.feeFloor = (*big.Int)(&gasPrice)
	api.e.lock.Unlock()

	api.e.txPool.SetFeeFloor((*big.Int)(&gasPrice))
	return true
}

// SetTauerbase sets the tauerbase of the miner
func (api *PrivateMinerAPI) SetTauerbase(tauerbase common.Address) bool {
	api.e.SetTauerbase(tauerbase)
	return true
}
...
// admin methods of the Tau service.
func NewPrivateAdminAPI(tau *Tau) *PrivateAdminAPI {
	return &PrivateAdminAPI{tau: tau}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.tau.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.tau.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.tau.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Tau full node APIs exposed
...
// related public debug methods of the Tau service.
func NewPublicDebugAPI(tau *Tau) *PublicDebugAPI {
	return &PublicDebugAPI{tau: tau}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.tau.miner.Pending()
		return stateDb.RawDump(false, false, true), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.tau.blockchain.CurrentBlock()
	} else {
		block = api.tau.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.tau.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Tau full node APIs exposed over
...
// private debug methods of the Tau service.
func NewPrivateDebugAPI(tau *Tau) *PrivateDebugAPI {
	return &PrivateDebugAPI{tau: tau}
}

// Preimage is a debug API function that returns the preimage for a sha3 hash, if known.
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.tau.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	blocks := api.tau.BlockChain().BadBlocks()
	...
	for i, block := range blocks {
		...
		if results[i].Block, err = tauapi.RPCMarshalBlock(block, true, true); err != nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.tau.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.tau.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.tau.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.tau.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.tau.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.tau.BlockChain().StateCache().TrieDB()
