...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Tau
...
func (tau *Tau) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-tau.shutdownChan:
					return

				case request := <-tau.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(tau.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(tau.chainDb, task.Bit, section, head); err == nil {
...
// for the Tau header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      taudb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db taudb.Database, size, confirms uint64) *core.ChainIndexer {
