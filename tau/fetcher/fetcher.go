...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (tau/61) or header (tau/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (tau/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both tau/61 and tau/62)
}
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somtauing went very wrong, drop the peer
