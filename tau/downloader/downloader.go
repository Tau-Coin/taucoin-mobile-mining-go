...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [tau/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB taudb.IpfsStore // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [tau/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [tau/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [tau/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [tau/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [tau/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [tau/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [tau/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb taudb.IpfsStore, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() tau.SyncProgress {
...
// Synchronising returns whtauer the downloader is currently retrieving blocks.
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "tau", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somtauing valid
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somtauing completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somtauing)
				// R: Nothing to give
				...
				// peer gave us somtauing useful, we're already happy/progressed (above check).
				if d.mode == FastSync {
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if somtauing failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCanceled
				default:
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put togtauer a new list of blocks to import, which is a superset
