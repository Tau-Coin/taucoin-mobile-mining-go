...
func (db *Database) NewIterator() taudb.Iterator {
...
// not exist).
func (db *Database) NewIteratorWithStart(start []byte) taudb.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
func (db *Database) NewIteratorWithPrefix(prefix []byte) taudb.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
// Next moves the iterator to the next key/value pair. It returns whtauer the
