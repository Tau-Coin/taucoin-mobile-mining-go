...
var (
	errFileOpen = errors.New("ipfsdb/storage: file still open")
	errReadOnly = errors.New("ipfsdb/storage: storage is read-only")
)
...
func OpenFile(path string, readOnly bool) (Storage, error) {
	if fi, err := os.Stat(path); err == nil {
		if !fi.IsDir() {
			return nil, fmt.Errorf("ipfsdb/storage: open %s: not a directory", path)
		}
...
func (fs *fileStorage) GetMeta() (FileDesc, error) {
	...
	tryCurrent := func(name string) (*currentFile, error) {
		...
		if len(b) < 1 || b[len(b)-1] != '\n' || !fsParseNamePtr(string(b[:len(b)-1]), &fd) {
			...
			err := &ErrCorrupted{
				Err: errors.New("ipfsdb/storage: corrupted or incomplete CURRENT file"),
			}
			return nil, err
		}
...
func (fw *fileWrap) Sync() error {
	...
	if fw.fd.Type == TypeManifest {
		...
		// See: https://code.google.com/p/ipfsdb/issues/detail?id=190.
