...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*tauapi.SignTransactionResult, error)
...
type UIClientAPI interface {
	...
	OnApprovedTx(tx tauapi.SignTransactionResult)
...
func logDiff(original *SignTxRequest, new *SignTxResponse) bool {
	...
	if g0, g1 := big.Int(original.Transaction.Fee), big.Int(new.Transaction.Fee); g0.Cmp(&g1) != 0 {
		modified = true
...
func (api *SignerAPI) lookupOrQueryPassword(address common.Address, title, prompt string) (string, error) {
	...
	if err != nil {
		...
		// which could leak the password if it was malformed json or somtauing
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*tauapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := tauapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
