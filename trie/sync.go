...
type request struct {
	...
	raw  bool        // Whtauer this is a raw entry (code) or a trie node
...
type Sync struct {
	database taudb.KeyValueReader     // Persistent database to check for existing entries
...
func (s *Sync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togtauer
...
// Process injects a batch of retrieved trie nodes data, returning if somtauing
...
func (s *Sync) Commit(dbw taudb.KeyValueWriter) (int, error) {
...
func (s *Sync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whtauer known or not
	type child struct {
		node  node
		depth int
	}
