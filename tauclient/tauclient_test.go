...
// Verify that Client implements the tau interfaces.
var (
	_ = tau.ChainReader(&Client{})
	_ = tau.TransactionReader(&Client{})
	_ = tau.ChainStateReader(&Client{})
	_ = tau.ChainSyncReader(&Client{})
	_ = tau.ContractCaller(&Client{})
	_ = tau.GasEstimator(&Client{})
	_ = tau.GasPricer(&Client{})
	_ = tau.LogFilterer(&Client{})
	_ = tau.PendingStateReader(&Client{})
	// _ = tau.PendingStateEventer(&Client{})
	_ = tau.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	n.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		config := &tau.Config{Genesis: genesis}
		config.Tauash.PowMode = tauhash.ModeFake
		tauservice, err = tau.New(ctx, config)
		return tauservice, err
	})
	...
	if _, err := tauservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func TestChainID(t *testing.T) {
	...
	if id == nil || id.Cmp(params.AllTauashProtocolChanges.ChainID) != 0 {
