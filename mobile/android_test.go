...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gtau.LightScryptN, Gtau.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.tau", new NodeConfig());
			node.start();
			...
			TauClient ec = node.getTauClient();
	...
	// signatures too. Regression test for go-tau issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogtauer
	...
	workspace, err := ioutil.TempDir("", "gtau-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/tau/gtautest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gtau and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.tau", "github.com/Tau-Coin/taucoin-mobile-mining-go/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gtau.aar"), "gtau.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "tau", "gtautest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
