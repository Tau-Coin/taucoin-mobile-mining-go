...
// Package graphql provides a GraphQL interface to Tau node data.
package graphql
...
// Account represents an Tau account at a particular block.
type Account struct {
	backend     tauapi.Backend
...
// Transaction represents an Tau transaction.
...
// `Log` objects.
func runFilter(ctx context.Context, be tauapi.Backend, filter *filters.Filter) ([]*Log, error) {
...
type CallData struct {
	From     *common.Address // The Tau address the call is from.
	To       *common.Address // The Tau address the call is to.
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, gas, failed, err := tauapi.DoCall(ctx, b.backend, args.Data, *b.num, nil, vm.Config{}, 5*time.Second, b.backend.RPCGasCap())
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	gas, err := tauapi.DoEstimateGas(ctx, b.backend, args.Data, *b.num, b.backend.RPCGasCap())
	return gas, err
}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	result, gas, failed, err := tauapi.DoCall(ctx, p.backend, args.Data, rpc.PendingBlockNumber, nil, vm.Config{}, 5*time.Second, p.backend.RPCGasCap())
	status := hexutil.Uint64(1)
	if failed {
		status = 0
	}
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	return tauapi.DoEstimateGas(ctx, p.backend, args.Data, rpc.PendingBlockNumber, p.backend.RPCGasCap())
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := tauapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
