...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Tau address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Tau account at a particular block.
    type Account {
    ...
    # Transaction is an Tau transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Tau account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Tau account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Tau block by number or by hash. If neither is
