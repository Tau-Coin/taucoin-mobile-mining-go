...
type Service struct {
	...
	backend  tauapi.Backend   // The backend that queries will operate onn.
	handler  http.Handler     // The `http.Handler` used to answer queries.
	listener net.Listener     // The listening socket.
}

// New constructs a new GraphQL service instance.
func New(backend tauapi.Backend, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) (*Service, error) {
	return &Service{
		endpoint: endpoint,
		cors:     cors,
		vhosts:   vhosts,
...
func newHandler(backend tauapi.Backend) (http.Handler, error) {
	q := Resolver{backend}
