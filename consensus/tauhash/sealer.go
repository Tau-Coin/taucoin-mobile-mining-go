...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid tauhash solution.
	staleThreshold = 7
)
...
func (tauhash *Tauash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if tauhash.config.PowMode == ModeFake || tauhash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", tauhash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if tauhash.shared != nil {
		return tauhash.shared.Seal(chain, block, results, stop)
	}
	...
	if tauhash.workCh != nil {
		tauhash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
...
search:
	for {
		...
		case <-abort:
			...
			tauhash.hashrate.Mark(attempts)
			break search
...
func (tauhash *Tauash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := tauhash.SealHash(block.Header())
	...
	// whtauer the solution was accepted or not (not can be both a bad pow as well as
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := tauhash.verifySeal(nil, header, true); err != nil {
		...
		if results == nil {
			log.Warn("Tauash result channel is empty, submitted mining result is rejected")
			return false
		}
	...
	for {
		...
		case result := <-tauhash.submitWorkCh:
		...
		case errc := <-tauhash.exitCh:
			// Exit remote loop if tauhash is closed and return relevant error.
			errc <- nil
			log.Trace("Tauash remote sealer is exiting")
			return
		}
	}
}
