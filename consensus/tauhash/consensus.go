...
var (
	...
	// bomb offset 5M.
	// Specification EIP-1234: https://eips.tau.org/EIPS/eip-1234
	calcDifficultyConstantinople = makeDifficultyCalculator(big.NewInt(5000000))

	// calcDifficultyByzantium is the difficulty adjustment algorithm. It returns
	// the difficulty that a new block should have when created at time given the
	// parent block's time and difficulty. The calculation uses the Byzantium rules.
	// Specification EIP-649: https://eips.tau.org/EIPS/eip-649
...
func (tauhash *Tauash) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	...
	if tauhash.config.PowMode == ModeFullFake {
		return nil
	}
...
func (tauhash *Tauash) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
	if tauhash.config.PowMode == ModeFullFake || len(headers) == 0 {
...
// rules of the stock Tau tauhash engine.
func (tauhash *Tauash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	// If we're running a full engine faking, accept any input as valid
	if tauhash.config.PowMode == ModeFullFake {
		return nil
	}
	...
	for _, uncle := range block.Uncles() {
		...
		if err := tauhash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, true); err != nil {
			return err
		}
	}
	return nil
...
func makeDifficultyCalculator(bombDelay *big.Int) func(time uint64, parent *types.Header) *big.Int {
	...
	return func(time uint64, parent *types.Header) *big.Int {
		// https://github.com/tau/EIPs/issues/100.
		// algorithm:
		// diff = (parent_diff +
		...
		// Specification: https://eips.tau.org/EIPS/eip-1234
...
func calcDifficultyHomestead(time uint64, parent *types.Header) *big.Int {
	// https://github.com/tau/EIPs/blob/master/EIPS/eip-2.md
	// algorithm:
	// diff = (parent_diff +
...
func (tauhash *Tauash) verifySeal(chain consensus.ChainReader, header *types.Header, fulldag bool) error {
	...
	if tauhash.config.PowMode == ModeFake || tauhash.config.PowMode == ModeFullFake {
		time.Sleep(tauhash.fakeDelay)
		if tauhash.fakeFail == header.Number.Uint64() {
	...
	if tauhash.shared != nil {
		return tauhash.shared.verifySeal(chain, header, fulldag)
	}
...
func (tauhash *Tauash) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) {
...
func (tauhash *Tauash) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	// Accumulate any block and uncle rewards and commit the final state root
	accumulateRewards(chain.Config(), state, header, uncles)
	header.Root = state.IntermediateRoot(chain.Config().IsEIP158(header.Number))
