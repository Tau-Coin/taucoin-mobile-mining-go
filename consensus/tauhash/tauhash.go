...
// Package tauhash implements the tauhash proof-of-work consensus engine.
package tauhash
...
// isLittleEndian returns whtauer the local system is running in little or big
// endian byte order.
...
func newlru(what string, maxItems int, new func(epoch uint64) interface{}) *lru {
	...
	cache, _ := simplelru.NewLRU(maxItems, func(key, value interface{}) {
		log.Trace("Evicted tauhash "+what, "epoch", key)
	})
...
func (lru *lru) get(epoch uint64) (item, future interface{}) {
	...
	if !ok {
		if lru.future > 0 && lru.future == epoch {
			...
			log.Trace("Requiring new tauhash "+lru.what, "epoch", epoch)
			item = lru.new(epoch)
		}
	...
	if epoch < maxEpoch-1 && lru.future < epoch+1 {
		log.Trace("Requiring new future tauhash "+lru.what, "epoch", epoch+1)
...
// cache wraps an tauhash cache with some metadata to allow easier concurrent use.
type cache struct {
...
// newCache creates a new tauhash verification cache and returns it as a plain Go
...
func (c *cache) generate(dir string, limit int, test bool) {
	c.once.Do(func() {
		...
		if err == nil {
			logger.Debug("Loaded old tauhash cache from disk")
			return
		}
		logger.Debug("Failed to load old tauhash cache", "err", err)

		// No previous cache available, create a new cache file to fill
		c.dump, c.mmap, c.cache, err = memoryMapAndGenerate(path, size, func(buffer []uint32) { generateCache(buffer, c.epoch, seed) })
		if err != nil {
			logger.Error("Failed to generate mapped tauhash cache", "err", err)
...
// dataset wraps an tauhash dataset with some metadata to allow easier concurrent use.
type dataset struct {
	epoch   uint64    // Epoch for which this cache is relevant
	dump    *os.File  // File descriptor of the memory mapped cache
	mmap    mmap.MMap // Memory map itself to unmap before releasing
...
// newDataset creates a new tauhash mining dataset and returns it as a plain Go
// interface to be usable in an LRU cache.
...
func (d *dataset) generate(dir string, limit int, test bool) {
	d.once.Do(func() {
		...
		if err == nil {
			logger.Debug("Loaded old tauhash dataset from disk")
			return
		}
		logger.Debug("Failed to load old tauhash dataset", "err", err)
		...
		if err != nil {
			logger.Error("Failed to generate mapped tauhash dataset", "err", err)
...
// generated returns whtauer this particular dataset finished generating already
...
func New(config Config, notify []string, noverify bool) *Tauash {
	...
	if config.CacheDir != "" && config.CachesOnDisk > 0 {
		log.Info("Disk storage enabled for tauhash caches", "dir", config.CacheDir, "count", config.CachesOnDisk)
	}
...
func (tauhash *Tauash) Close() error {
	var err error
	tauhash.closeOnce.Do(func() {
		// Short circuit if the exit channel is not allocated.
		if tauhash.exitCh == nil {
			return
		}
		errc := make(chan error)
		tauhash.exitCh <- errc
		err = <-errc
		close(tauhash.exitCh)
	})
	return err
}
...
func (tauhash *Tauash) cache(block uint64) *cache {
	epoch := block / epochLength
	currentI, futureI := tauhash.caches.get(epoch)
	current := currentI.(*cache)

	// Wait for generation finish.
	current.generate(tauhash.config.CacheDir, tauhash.config.CachesOnDisk, tauhash.config.PowMode == ModeTest)

	// If we need a new future cache, now's a good time to regenerate it.
	if futureI != nil {
		future := futureI.(*cache)
		go future.generate(tauhash.config.CacheDir, tauhash.config.CachesOnDisk, tauhash.config.PowMode == ModeTest)
	}
	return current
}
...
func (tauhash *Tauash) dataset(block uint64, async bool) *dataset {
	// Retrieve the requested tauhash dataset
	epoch := block / epochLength
	currentI, futureI := tauhash.datasets.get(epoch)
	current := currentI.(*dataset)

	// If async is specified, generate everything in a background thread
	if async && !current.generated() {
		go func() {
			current.generate(tauhash.config.DatasetDir, tauhash.config.DatasetsOnDisk, tauhash.config.PowMode == ModeTest)

			if futureI != nil {
				future := futureI.(*dataset)
				future.generate(tauhash.config.DatasetDir, tauhash.config.DatasetsOnDisk, tauhash.config.PowMode == ModeTest)
			}
		}()
	} else {
		...
		if futureI != nil {
			future := futureI.(*dataset)
			go future.generate(tauhash.config.DatasetDir, tauhash.config.DatasetsOnDisk, tauhash.config.PowMode == ModeTest)
		}
	}
	return current
}
...
func (tauhash *Tauash) Threads() int {
	tauhash.lock.Lock()
	defer tauhash.lock.Unlock()

	return tauhash.threads
}
...
// work being done.
func (tauhash *Tauash) SetThreads(threads int) {
	tauhash.lock.Lock()
	defer tauhash.lock.Unlock()
...
func (tauhash *Tauash) APIs(chain consensus.ChainReader) []rpc.API {
	// In order to ensure backward compatibility, we exposes tauhash RPC APIs
	// to both tau and tauhash namespaces.
	return []rpc.API{
		{
			Namespace: "tau",
			Version:   "1.0",
