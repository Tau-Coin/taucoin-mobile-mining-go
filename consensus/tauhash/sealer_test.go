...
// Tests whtauer remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom tauhash engine
	tauhash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer tauhash.Close()
	...
	tauhash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := tauhash.SealHash(header).Hex(); work[0] != want {
...
// Tests whtauer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	tauhash := NewTester(nil, true)
	defer tauhash.Close()
	api := &API{tauhash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			tauhash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, tauhash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch tauhash result timeout", id+1)
		}
	}
}
