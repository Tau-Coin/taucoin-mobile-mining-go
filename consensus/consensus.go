...
// Package consensus implements different Tau consensus engines.
package consensus
...
type Engine interface {
	// Author retrieves the Tau address of the account that minted the given
	// block, which may be different from the header's coinbase if a consensus
	// engine is based on signatures.
	Author(header *types.Header) (common.Address, error)

	// VerifyHeader checks whtauer a header conforms to the consensus rules of a
	...
	// VerifySeal checks whtauer the crypto seal on a header is valid according to
	...
	// Note: The block header and state database might be updated to reflect any
	// consensus rules that happen at finalization (e.g. block rewards).
	FinalizeAndAssemble(chain ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction) (*types.Block, error)
