...
// mapped from textual names used in the tests below to actual Tau private
...
// address retrieves the Tau address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Tau address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/Tau-Coin/taucoin-mobile-mining-go/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Gtau the header and prepare it for signing
			header := block.Header()
			if j > 0 {
