...
var (
	...
	// errInvalidVote is returned if a nonce value is somtauing else that the two
...
// ecrecover extracts the Tau account address from a signed header.
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, error) {
	...
	// Recover the public key and the Tau address
...
// Tau testnet following the Ropsten attacks.
type Clique struct {
	config *params.CliqueConfig // Consensus engine configuration parameters
	db     taudb.Database       // Database to store and retrieve snapshot checkpoints
	...
	signer common.Address // Tau address of the signing key
...
func New(config *params.CliqueConfig, db taudb.Database) *Clique {
...
// Author implements consensus.Engine, returning the Tau address recovered
// from the signature in the header's extra-data section.
func (c *Clique) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures)
}

// VerifyHeader checks whtauer a header conforms to the consensus rules.
...
// verifyHeader checks whtauer a header conforms to the consensus rules.The
...
// verifySeal checks whtauer the signature contained in the header satisfies the
