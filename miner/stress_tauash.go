...
// This file contains a miner stress test based on the Tauash consensus engine.
package main
...
func main() {
	...
	// Pre-generate the tauhash mining DAG so we don't race
	tauhash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".tauhash"))

	// Create an Tauash network based off of the Ropsten config
	...
	for _, node := range nodes {
		var tau *tau.Tau
		if err := node.Service(&tau); err != nil {
			panic(err)
		}
		if err := tau.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var tau *tau.Tau
		if err := nodes[index%len(nodes)].Service(&tau); err != nil {
			panic(err)
		}
		...
		if err := tau.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := tau.TxPool().Stats(); pend > 2048 {
			time.Sleep(100 * time.Millisecond)
		}
	}
}

// makeGenesis creates a custom Tauash genesis block based on some pre-defined
// faucet accounts.
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Tau node
	...
	// Start the node and configure a full Tau node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, err
	}
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return tau.New(ctx, &tau.Config{
			Genesis:         genesis,
			...
			GPO:             tau.DefaultConfig.GPO,
			Tauash:          tau.DefaultConfig.Tauash,
			Miner: Config{
