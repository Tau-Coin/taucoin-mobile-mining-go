...
// starting nodes and connecting them togtauer.
//
...
// Connect connects two nodes togtauer by calling the "admin_addPeer" RPC
...
// NOTE:
// it also checks whtauer there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whtauer or not the node is running
	up   bool
	upMu sync.RWMutex
}
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/tauersphere/go-tau/issues/1177
	node := struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}{}
...
type Conn struct {
	...
	// Up tracks whtauer or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whtauer both nodes are currently up
