...
creating a collection of devp2p nodes, connecting them togtauer to form a
...
connecting two nodes togtauer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somtauing happening in the network as a result of a
...
* `Expect` - an expectation function which returns whtauer or not a
...
As a concrete example, consider a simulated network of Tau nodes. An
