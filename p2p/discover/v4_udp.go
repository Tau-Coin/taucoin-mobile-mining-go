...
type packetV4 interface {
	// preverify checks whtauer the packet is valid and should be handled at all.
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whtauer the callback should
...
type reply struct {
	...
	ip   net.IP
	data packetV4
	// loop indicates whtauer there was
...
// whtauer any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whtauer any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
...
// expired checks whtauer the given UNIX time stamp is in the past.
