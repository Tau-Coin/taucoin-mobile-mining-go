...
// PublicTauAPI provides an API to access Tau related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicTauAPI struct {
	b Backend
}

// NewPublicTauAPI creates a new Tau protocol API.
func NewPublicTauAPI(b Backend) *PublicTauAPI {
	return &PublicTauAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicTauAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current Tau protocol version this node supports
func (s *PublicTauAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicTauAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Tau ECDSA signature for:
// keccack256("\x19Tau Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/Tau-Coin/taucoin-mobile-mining-go/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with tau_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Tau Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/Tau-Coin/taucoin-mobile-mining-go/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Tau signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gtau.
...
// PublicBlockChainAPI provides an API to access the Tau blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Tau blockchain API.
...
//
// https://github.com/tau/wiki/wiki/JSON-RPC#tau_sign
...
// PublicDebugAPI is the collection of Tau APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", tauhash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Tau APIs exposed over the private
// debugging endpoint.
...
// Version returns the current tau protocol version.
