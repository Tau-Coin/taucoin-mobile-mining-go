...
// gtau is the official command-line client for Tau.
package main
...
const (
	clientIdentifier = "gtau" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, gitDate, "the go-tau command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.TauashDatasetDirFlag,
		utils.TauashDatasetsInMemoryFlag,
		utils.TauashDatasetsOnDiskFlag,
...
func init() {
	// Initialize the CLI app and start Gtau
	app.Action = gtau
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2019 The go-tau Authors"
...
// gtau is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gtau(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local gtau node.
	...
	go func() {
		...
		for event := range events {
			...
			case accounts.WalletOpened:
				...
				event.Wallet.SelfDerive(derivationPaths, tauClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Tau node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
