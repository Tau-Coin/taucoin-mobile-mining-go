...
func TestAccountListEmpty(t *testing.T) {
	gtau := runGtau(t, "account", "list")
	gtau.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gtau := runGtau(t, "account", "list", "--datadir", datadir)
	defer gtau.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gtau := runGtau(t, "account", "new", "--lightkdf")
	defer gtau.ExpectExit()
	gtau.Expect(`
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "somtauing"}}
Repeat password: {{.InputLine "somtauing else"}}
...
func TestWalletImport(t *testing.T) {
	gtau := runGtau(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gtau.ExpectExit()
	gtau.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gtau.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gtau.StderrText(), m) {
...
// https://github.com/Tau-Coin/taucoin-mobile-mining-go/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gtau.SetTemplateFunc("keypath", func(file string) string {
