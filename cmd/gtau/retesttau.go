...
var (
	...
	retesttauCommand = cli.Command{
		Action:      utils.MigrateFlags(retesttau),
		Name:        "retesttau",
		Usage:       "Launches gtau in retesttau mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches gtau in retesttau mode (no database, no network, only retesttau RPC interface)`,
	}
)

type RetesttauTestAPI interface {
...
type RetesttauTauAPI interface {
...
type RetesttauDebugAPI interface {
...
func (api *RetesttauAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	// Clean up
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(tauDb, genesis)
	if err != nil {
		return false, err
	}
	...
	case "NoProof", "NoReward":
		inner = tauhash.NewFaker()
	case "Tauash":
		inner = tauhash.New(tauhash.Config{
			CacheDir:       "tauhash",
			CachesInMem:    2,
			CachesOnDisk:   3,
			DatasetsInMem:  1,
...
func (api *RetesttauAPI) SendRawTransaction(ctx context.Context, rawTx hexutil.Bytes) (common.Hash, error) {
...
func (api *RetesttauAPI) mineBlock() error {
	parentHash := rawdb.ReadCanonicalHash(api.tauDb, api.blockNumber)
	parent := rawdb.ReadBlock(api.tauDb, parentHash, api.blockNumber)
...
func (api *RetesttauAPI) importBlock(block *types.Block) error {
...
func (api *RetesttauAPI) ModifyTimestamp(ctx context.Context, interval uint64) (bool, error) {
	api.blockInterval = interval
	return true, nil
}

func (api *RetesttauAPI) ImportRawBlock(ctx context.Context, rawBlock hexutil.Bytes) (common.Hash, error) {
...
func (api *RetesttauAPI) RewindToBlock(ctx context.Context, newHead uint64) (bool, error) {
...
func (api *RetesttauAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.tauDb, txHash, api.chainConfig)
	if receipt == nil {
...
func (api *RetesttauAPI) BlockNumber(ctx context.Context) (uint64, error) {
	//fmt.Printf("BlockNumber, response: %d\n", api.blockNumber)
	return api.blockNumber, nil
}

func (api *RetesttauAPI) GetBlockByNumber(ctx context.Context, blockNr math.HexOrDecimal64, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetesttauAPI) AccountRange(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	addressHash *math.HexOrDecimal256, maxResults uint64,
) (AccountRangeResult, error) {
	var (
...
func (api *RetesttauAPI) GetBalance(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (*math.HexOrDecimal256, error) {
...
func (api *RetesttauAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Gtau-" + params.VersionWithCommit(gitCommit, gitDate), nil
}
...
func retesttau(ctx *cli.Context) error {
	log.Info("Welcome to retesttau!")
	// register signer API with server
	var (
		extapiURL string
	)
	apiImpl := &RetesttauAPI{}
	var testApi RetesttauTestAPI = apiImpl
	var tauApi RetesttauTauAPI = apiImpl
	var debugApi RetesttauDebugAPI = apiImpl
	...
	listener, _, err := rpc.StartHTTPEndpoint(httpEndpoint, rpcAPI, []string{"test", "tau", "debug", "web3"}, cors, vhosts, rpc.DefaultHTTPTimeouts)
	if err != nil {
