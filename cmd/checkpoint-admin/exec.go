...
import (
	...
	"github.com/Tau-Coin/taucoin-mobile-mining-go/contracts/checkpointoracle"
...
func sign(ctx *cli.Context) error {
	var (
		offline bool // The indicator whtauer we sign checkpoint by offline.
		chash   common.Hash
		cindex  uint64
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		head, err := tauclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
	...
	// isAdmin checks whtauer the specified signer is admin.
...
func publish(ctx *cli.Context) error {
	...
	head, err := tauclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := tauclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
