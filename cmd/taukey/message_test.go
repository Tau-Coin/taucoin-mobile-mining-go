...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "taukey-test")
	if err != nil {
	...
	// Create the key.
	generate := runTaukey(t, "generate", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runTaukey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runTaukey(t, "verifymessage", address, signature, message)
