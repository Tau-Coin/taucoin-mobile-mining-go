...
// Package utils contains internal helper functions for go-tau commands.
package utils
...
import (
	...
	"github.com/Tau-Coin/taucoin-mobile-mining-go/graphql"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: tau.DefaultConfig.NetworkId,
	}
	...
	LightLegacyServFlag = cli.IntFlag{ // Deprecated in favor of light.serve, remove in 2021
		...
		Value: tau.DefaultConfig.LightServ,
	}
	...
	UltraLightServersFlag = cli.StringFlag{
		...
		Value: strings.Join(tau.DefaultConfig.UltraLightServers, ","),
	}
	UltraLightFractionFlag = cli.IntFlag{
		Name:  "ulc.fraction",
		Usage: "Minimum % of trusted ultra-light servers required to announce a new head",
		Value: tau.DefaultConfig.UltraLightFraction,
	}
	...
	TauashCacheDirFlag = DirectoryFlag{
		Name:  "tauhash.cachedir",
		Usage: "Directory to store the tauhash verification caches (default = inside the datadir)",
	}
	TauashCachesInMemoryFlag = cli.IntFlag{
		Name:  "tauhash.cachesinmem",
		Usage: "Number of recent tauhash caches to keep in memory (16MB each)",
		Value: tau.DefaultConfig.Tauash.CachesInMem,
	}
	TauashCachesOnDiskFlag = cli.IntFlag{
		Name:  "tauhash.cachesondisk",
		Usage: "Number of recent tauhash caches to keep on disk (16MB each)",
		Value: tau.DefaultConfig.Tauash.CachesOnDisk,
	}
	TauashDatasetDirFlag = DirectoryFlag{
		Name:  "tauhash.dagdir",
		Usage: "Directory to store the tauhash mining DAGs",
		Value: DirectoryString(tau.DefaultConfig.Tauash.DatasetDir),
	}
	TauashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "tauhash.dagsinmem",
		Usage: "Number of recent tauhash mining DAGs to keep in memory (1+GB each)",
		Value: tau.DefaultConfig.Tauash.DatasetsInMem,
	}
	TauashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "tauhash.dagsondisk",
		Usage: "Number of recent tauhash mining DAGs to keep on disk (1+GB each)",
		Value: tau.DefaultConfig.Tauash.DatasetsOnDisk,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: tau.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: tau.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: tau.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: tau.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: tau.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: tau.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: tau.DefaultConfig.TxPool.Lifetime,
	}
	...
	RPCGlobalGasCap = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in tau_call/estimateGas",
	}
...
func setLes(ctx *cli.Context, cfg *tau.Config) {
	...
	if cfg.UltraLightFraction <= 0 && cfg.UltraLightFraction > 100 {
		log.Error("Ultra light fraction is invalid", "had", cfg.UltraLightFraction, "updated", tau.DefaultConfig.UltraLightFraction)
		cfg.UltraLightFraction = tau.DefaultConfig.UltraLightFraction
	}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gtau account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// setTauerbase retrieves the tauerbase either from the directly specified
...
func setTauerbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *tau.Config) {
	...
	// Convert the tauerbase into an address and configure it
	if tauerbase != "" {
		if ks != nil {
			account, err := MakeAddress(ks, tauerbase)
			if err != nil {
				Fatalf("Invalid miner tauerbase: %v", err)
			}
			cfg.Miner.Tauerbase = account.Address
		} else {
			Fatalf("No tauerbase configured")
		}
	}
}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	tauPeers := cfg.MaxPeers - lightPeers
	if lightClient {
		tauPeers = 0
	}
	log.Info("Maximum peer count", "TAU", tauPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func setTauash(ctx *cli.Context, cfg *tau.Config) {
	if ctx.GlobalIsSet(TauashCacheDirFlag.Name) {
		cfg.Tauash.CacheDir = ctx.GlobalString(TauashCacheDirFlag.Name)
	}
	if ctx.GlobalIsSet(TauashDatasetDirFlag.Name) {
		cfg.Tauash.DatasetDir = ctx.GlobalString(TauashDatasetDirFlag.Name)
	}
	if ctx.GlobalIsSet(TauashCachesInMemoryFlag.Name) {
		cfg.Tauash.CachesInMem = ctx.GlobalInt(TauashCachesInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(TauashCachesOnDiskFlag.Name) {
		cfg.Tauash.CachesOnDisk = ctx.GlobalInt(TauashCachesOnDiskFlag.Name)
	}
	if ctx.GlobalIsSet(TauashDatasetsInMemoryFlag.Name) {
		cfg.Tauash.DatasetsInMem = ctx.GlobalInt(TauashDatasetsInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(TauashDatasetsOnDiskFlag.Name) {
		cfg.Tauash.DatasetsOnDisk = ctx.GlobalInt(TauashDatasetsOnDiskFlag.Name)
	}
}
...
// RegisterTauStatsService configures the Tau Stats daemon and adds it to
// the given node.
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		var tauServ *tau.Tau
		if err := ctx.Service(&tauServ); err == nil {
			return graphql.New(tauServ.APIBackend, endpoint, cors, vhosts, timeouts)
		}
		...
		return nil, errors.New("no Tau service")
	}); err != nil {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gtau.", tagsMap)
		}
	}
}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb taudb.Database) {
	...
	cache := &core.CacheConfig{
		TrieCleanLimit:      tau.DefaultConfig.TrieCleanCache,
		TrieCleanNoPrefetch: ctx.GlobalBool(CacheNoPrefetchFlag.Name),
		TrieDirtyLimit:      tau.DefaultConfig.TrieDirtyCache,
		TrieDirtyDisabled:   ctx.GlobalString(GCModeFlag.Name) == "archive",
		TrieTimeLimit:       tau.DefaultConfig.TrieTimeout,
	}
	if ctx.GlobalIsSet(CacheFlag.Name) || ctx.GlobalIsSet(CacheTrieFlag.Name) {
...
// new format.
//
// e.g. gtau account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gtau --keystore /tmp/mykeystore --lightkdf account new
//
