...
// makeWizard creates and returns a new pupptau wizard.
...
// setting up a new or managing an existing Tau private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to pupptau, your Tau private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Tau network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and taustats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Pupptau uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Tau network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".pupptau", w.network)
