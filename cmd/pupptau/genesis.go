...
// altauGenesisSpec represents the genesis specification format used by the
// C++ Tau implementation.
type altauGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*altauGenesisSpecAccount `json:"accounts"`
}

// altauGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type altauGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *altauGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// altauGenesisSpecBuiltin is the precompiled contract definition.
type altauGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                 `json:"startingBlock,omitempty"`
	Linear        *altauGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type altauGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAltauGenesisSpec converts a go-tau genesis block into a Altau-specific
// chain specification format.
func newAltauGenesisSpec(network string, genesis *core.Genesis) (*altauGenesisSpec, error) {
	// Only tauhash is currently supported between go-tau and altau
	if genesis.Config.Tauash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Altau format
	spec := &altauGenesisSpec{
		SealEngine: "Tauash",
	}
	// Some defaults
	...
	spec.Params.BlockReward = (*hexutil.Big)(tauhash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &altauGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &altauGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &altauGenesisSpecBuiltin{Name: "sha256",
		Linear: &altauGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &altauGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &altauGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &altauGenesisSpecBuiltin{Name: "identity",
		Linear: &altauGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &altauGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
		spec.setPrecompile(6, &altauGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &altauGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &altauGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &altauGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &altauGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
	}
	return spec, nil
}

func (spec *altauGenesisSpec) setPrecompile(address byte, data *altauGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*altauGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &altauGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *altauGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*altauGenesisSpecAccount)
	}
...
func (spec *altauGenesisSpec) setByzantium(num *big.Int) {
	spec.Params.ByzantiumForkBlock = hexutil.Uint64(num.Uint64())
}

func (spec *altauGenesisSpec) setConstantinople(num *big.Int) {
...
// newParityChainSpec converts a go-tau genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only tauhash is currently supported between go-tau and Parity
	if genesis.Config.Tauash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Engine.Tauash.Params.BlockReward = make(map[string]string)
	spec.Engine.Tauash.Params.DifficultyBombDelays = make(map[string]string)
	// Frontier
	spec.Engine.Tauash.Params.MinimumDifficulty = (*hexutil.Big)(params.MinimumDifficulty)
	spec.Engine.Tauash.Params.DifficultyBoundDivisor = (*hexutil.Big)(params.DifficultyBoundDivisor)
	spec.Engine.Tauash.Params.DurationLimit = (*hexutil.Big)(params.DurationLimit)
	spec.Engine.Tauash.Params.BlockReward["0x0"] = hexutil.EncodeBig(tauhash.FrontierBlockReward)

	// Homestead
	spec.Engine.Tauash.Params.HomesteadTransition = hexutil.Uint64(genesis.Config.HomesteadBlock.Uint64())

	// Tangerine Whistle : 150
	// https://github.com/tau/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/tau/EIPs/blob/master/EIPS/eip-607.md
	...
	// gtau has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Tau.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Tau.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Tau.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Tauash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(tauhash.ByzantiumBlockReward)
	spec.Engine.Tauash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(3000000)
	n := hexutil.Uint64(num.Uint64())
	spec.Engine.Tauash.Params.EIP100bTransition = n
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Tauash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(tauhash.ConstantinopleBlockReward)
	spec.Engine.Tauash.Params.DifficultyBombDelays[hexutil.EncodeBig(num)] = hexutil.EncodeUint64(2000000)
	n := hexutil.Uint64(num.Uint64())
...
// pyTauGenesisSpec represents the genesis specification format used by the
// Python Tau implementation.
type pyTauGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyTauGenesisSpec converts a go-tau genesis block into a Parity specific
// chain specification format.
func newPyTauGenesisSpec(network string, genesis *core.Genesis) (*pyTauGenesisSpec, error) {
	// Only tauhash is currently supported between go-tau and pytau
	if genesis.Config.Tauash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyTauGenesisSpec{
