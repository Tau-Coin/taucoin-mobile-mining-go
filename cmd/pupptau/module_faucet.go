...
FROM tau/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--taustats", "{{.Taustats}}", "--tauport", "{{.TauPort}}",     \
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d Tauers", info.amount),
...
// checkFaucet does a health-check against a faucet server to verify whtauer
