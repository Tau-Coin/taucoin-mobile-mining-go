...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. Tauash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of tauhash, we're pretty much done
		genesis.Config.Tauash = new(params.TauashConfig)
...
// importGenesis imports a Gtau genesis spec into pupptau.
...
func (w *wizard) manageGenesis() {
	// Figure out whtauer to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-altau.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by pupptau and Gtau
		gtauJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gtauJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gtauJson)

		// Export the genesis spec used by Altau (formerly C++ Tau)
		if spec, err := newAltauGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Altau chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "altau", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly TauJ
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somtauing I can do")
		return
	}
}
