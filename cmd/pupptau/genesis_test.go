...
// Tests the go-tau to Altau chainspec conversion for the Stureby testnet.
func TestAltauSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gtau.json")
	if err != nil {
	...
	spec, err := newAltauGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_altau.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &altauGenesisSpec{}
...
// Tests the go-tau to Parity chainspec conversion for the Stureby testnet.
