...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Tauers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.taustats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.taustats = w.readString() + ":" + w.conf.taustats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.taustats)
		infos.node.taustats = w.readDefaultString(infos.node.taustats) + ":" + w.conf.taustats
	}
