...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for taustats availability")
	if infos, err := checkTaustats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["taustats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["taustats"] = infos.Report()
		taustats = infos.config
	}
