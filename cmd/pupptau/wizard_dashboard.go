...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"taustats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "taustats":
				if infos, err := checkTaustats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have taustats running, ask whtauer to make the secret public or not
	if w.conf.taustats != "" {
		fmt.Println()
		fmt.Println("Include taustats secret on dashboard (y/n)? (default = yes)")
