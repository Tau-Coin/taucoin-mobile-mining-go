...
FROM pupptau/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gtau --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gtau --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.TauPort}} --bootnodes {{.Bootnodes}} --taustats \'{{.Taustats}}\' --cache=512 --rpc --rpcapi "net,web3,tau,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gtau --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.TauPort}} --bootnodes {{.Bootnodes}} --taustats \'{{.Taustats}}\' --cache=512 --rpc --rpcapi "net,web3,tau,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.TauPort}}:{{.TauPort}}"
            - "{{.TauPort}}:{{.TauPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - TAU_PORT={{.TauPort}}
            - TAU_NAME={{.TauName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Taustats":  config.node.taustats,
		"TauPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"TauPort":     config.node.port,
		"TauName":     config.node.taustats[:strings.Index(config.node.taustats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Tau listener port ": strconv.Itoa(info.node.port),
		"Taustats username":       info.node.taustats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["TAU_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.tau"],
			port:     infos.portmap[infos.envvars["TAU_PORT"]+"/tcp"],
			taustats: infos.envvars["TAU_NAME"],
		},
