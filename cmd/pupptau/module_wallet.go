...
FROM pupptau/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gtau --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gtau --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --taustats \'{{.Taustats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PupptauNetworkID/{{.NetworkID}}/g' dist/js/tauerwallet-master.js && \
	sed -i 's/PupptauNetwork/{{.Network}}/g'     dist/js/tauerwallet-master.js && \
	sed -i 's/PupptauDenom/{{.Denom}}/g'         dist/js/tauerwallet-master.js && \
	sed -i 's/PupptauHost/{{.Host}}/g'           dist/js/tauerwallet-master.js && \
	sed -i 's/PupptauRPCPort/{{.RPCPort}}/g'     dist/js/tauerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whtauer
// it's running, and if yes, whtauer it's responsive.
