...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.taustats == "" {
		log.Error("No taustats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Tauash != nil && !boot {
		fmt.Println()
		if infos.tauhashdir == "" {
			fmt.Printf("Where should the tauhash mining DAGs be stored on the remote machine?\n")
			infos.tauhashdir = w.readString()
		} else {
			fmt.Printf("Where should the tauhash mining DAGs be stored on the remote machine? (default = %s)\n", infos.tauhashdir)
			infos.tauhashdir = w.readDefaultString(infos.tauhashdir)
		}
	}
	...
	if infos.taustats == "" {
		...
		infos.taustats = w.readString() + ":" + w.conf.taustats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.taustats)
		infos.taustats = w.readDefaultString(infos.taustats) + ":" + w.conf.taustats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Tauash != nil {
			// Tauash based miners only need an tauerbase to mine against
			fmt.Println()
			if infos.tauerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.tauerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.tauerbase)
				infos.tauerbase = w.readDefaultAddress(common.HexToAddress(infos.tauerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Tau node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
