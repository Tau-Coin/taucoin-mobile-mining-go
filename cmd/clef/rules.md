...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 tauer` in value to maximum `1 tauer` per 24h period
* I want to auto-approve transaction to contract `TauAlarmClock` with `data`=`0xdeadbeef`, if `value=0`, `gas < 44k` and `gasPrice < 40Gwei`
...
2. JS returns "Reject"
  * Auto-reject request
3. Error occurs, or somtauing else is returned
...
implemented for `gtau`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
It's unclear whtauer any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
